 ...
 * @Last Modified by: wu
 * @Last Modified time: 2018-01-31 11:10:44
 */
...
app.controller('reservationRestimeCtrl', function($scope,$interval,httpService,$stateParams,$filter) {
	...
	for(var i = 0; i < 7; i++){
		...
		$scope.data.date_week.push({val:temp_date_val,date:temp_date,week:zhou});
	}
 ...
 * 预约挂号  科室
 */
app.controller('reservationRestimeDepartmentTransitRegisteredDocinfoDepCtrl', function($scope,$interval,$timeout,httpService,$filter) {
	...
	//   金额转分
	$scope.item.money_minute = +$scope.item.visitCost ? (+$scope.item.visitCost).toFixed(2) * 100 : 0;
	//   序号转换
	...
	var orderPayCompleted = function(){
		...
		$scope.form_data_completed.payTradeNo = $scope.bankRechargeObj.Trace_no;
		//
			...
			if(res.succeed){
				...
				orderDetailInfo($scope.res_SubmitOrder.orderId, function(res_order_info){
					if(res_order_info.succeed){
						...
						//   预约失败
						$scope.systemError('预约失败-请联系维护人员');
						//   打印支付成功预约失败凭条
	...
	$scope.statusFn2 = function(){
		...
		var temp_obj = {
			...
			visitCost: $scope.res_orderPayCompleted.visitCost,
		...
		httpService.ajaxPost(httpService.API.href + '/api/yytMace/v1/yyPrint', temp_obj)
			.then(function (res) {
				...
				if (res.succeed) {
					...
					//    开始打印
					var temp_str = '----------------------------------';
					if($scope.bankCorrectStatus != 0 && res.data.print.indexOf(temp_str) > -1){
