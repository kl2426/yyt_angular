 ...
 * @Last Modified by: wu
 * @Last Modified time: 2018-06-01 09:27:36
 */
'use strict';
 ...
 * 取网络当班挂号凭条
 */
app.controller('reservationDepartmentCtrl', function($scope, $interval, $timeout, $filter, httpService, globalFn) {
					...
					for(var i in $scope.data.items) {
						...
						ticketQuery($scope.data.items[i], function() {
							...
							if(ticketQuery_index <= 0) {
								...
								outAllFn();
								//   选中全部未取的
								for (var h in $scope.data.items){
									if ($scope.data.items[h].has_print == 0){
										$scope.data.items[h].checked = true;
									}
								}
								//
								//
		...
		for(var i in $scope.data.checked_items) {
			if($scope.data.checked_items[i].has_print == 1) {
				...
				temp_obj.detailed = JSON.stringify([{
					'floor_name': $scope.data.checked_items[i].floor_name
				}])
				//
				//  调用打印凭条接口
				zzghPrint(temp_obj);
