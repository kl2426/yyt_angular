...
app.controller('cardDepartmentRegisteredCtrl', function($scope, $interval, httpService, $stateParams) {
		...
		httpService.ajaxGet(httpService.API.href + '/api/yytBase/v1/doctor/' + $stateParams.deptCode)
			.then(function(res) {
				...
				if(res.succeed) {
					...
					//   验证医生没有当班信息
					// var index_length = $scope.data.items.length;
					// for (var i in $scope.data.items){
					// 	schedul($scope.data.items[i], function (res_item, res_schedul) {
					// 		index_length = index_length - 1;
					// 		if(res_schedul.length > 0){
					// 			res_item.has_schedul = true;
					// 		}else{
					// 			res_item.has_schedul = false;
					// 		}
					// 		//  最后一条
					// 		if (index_length <= 0){
					// 			$scope.items_loadding = false;
	...
	// //   取医生当班信息
	// var schedul = function (item, cb) {
	// 	$scope.items_loadding = true;
	// 	//  
	// 	httpService.ajaxGet(httpService.API.href + '/api/yytBase/v1/schedul/' + item.doct_code + '/' + item.dept_code)
	// 		.then(function (res) {
	// 			//
	// 			if (res.succeed) {
	// 				if (res.data.item.__proto__.constructor == Array) {
	...
	// 					res.data.item = [res.data.item];
	// 				}
	// 				typeof cb == "function" && cb(item, res.data.item);
	// 			} else {
