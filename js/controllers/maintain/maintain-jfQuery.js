 ...
 *  维护人员相关界面 - 缴费凭条补打
 */
app.controller('modalMaintainJfQueryCtrl', function($scope, $interval, $modal, $modalInstance, items, $filter, httpService, $timeout, globalFn) {
	...
	$scope.isPrint = function() {
		...
		var prescRecordID = temp_prescRecordID.length > 0 ? temp_prescRecordID.join('-') : '';
		//
		getIsPay($scope.items.scope.app.user_info.card_no, prescRecordID);
	}

	//	处方缴费结果确认
	var getIsPay = function(cardNo, prescRecordID) {
		var temp_obj = {
			cardNo: cardNo,
			prescRecordID: prescRecordID
		};

		httpService.adminAjaxPost(httpService.API.href + '/api/yytBack/v1/getIsPay', temp_obj)
			.then(function(res) {
				if(res.succeed) {
					//	成功
					$scope.data.chargeSuccessed = true;
				} else {
					//  失败
					$scope.items.scope.systemError('处方缴费失败：请与工作人员联系！');
				}
			});
	}
